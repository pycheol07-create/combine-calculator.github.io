...
import { 
    DOCS_FEE, CO_FEE, OCEAN_FREIGHT_RATE_PER_CBM, 
    CBM_WEIGHT_DIVISOR, VAT_RATE 
} from '../constants.js';
import { InputControl } from '../components/InputControl.js';
import { LiveRateDisplay } from '../components/LiveRateDisplay.js';
import { ResultCard } from '../components/ResultCard.js';
import { AnalysisModal } from '../components/AnalysisModal.js';
import * as Icons from '../components/Icons.js'; // 아이콘 가져오기
...
export const CustomsCalculator = ({ exchangeRate, onExchangeRateChange, onSaveCompare }) => {
    ...
    const productModeFields = [
      { label: "상품 수량", name: "productQuantity", unit: "개", icon: <Icons.CalculatorIcon /> },
      { label: "상품 단가", name: "unitPrice", unit: "USD/개", icon: <Icons.CurrencyDollarIcon /> },
      { label: "박스당 수량", name: "quantityPerBox", unit: "개/박스", icon: <Icons.BoxIcon /> },
    ];
    const boxModeFields = [
      { label: "총 박스 수량", name: "boxQuantity", unit: "박스", icon: <Icons.BoxIcon /> },
      { label: "총 상품 금액", name: "totalProductPrice", unit: "USD", icon: <Icons.CurrencyDollarIcon /> },
    ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mt-8">
          ...
          <div ref={formRef} className="bg-gradient-to-br from-emerald-50/60 to-white/60 backdrop-blur-xl p-6 md:p-8 rounded-2xl shadow-lg border border-slate-200">
            ...
            <div className="space-y-6">
              ...
              {/* Weight */}
              <InputControl label="박스당 무게" name="weightPerBox" value={formData.weightPerBox} onChange={handleInputChange} unit="kg/박스" icon={<Icons.ScaleIcon />} onKeyDown={handleKeyDown} />

              {/* Exchange Rate */}
              <div>
                  <InputControl label="고시환율" name="exchangeRate" value={exchangeRate} onChange={(e) => onExchangeRateChange(e.target.value)} unit="원-달러" icon={<Icons.TrendingUpIcon />} onKeyDown={handleKeyDown} />
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-x-2">
                    ...
                    <InputControl 
                      ...
                      icon={formData.commissionType === 'percentage' ? <Icons.PercentageIcon /> : <Icons.CurrencyWonIcon />} 
              ...
              <div className={`grid transition-all duration-300 ease-in-out ${formData.shippingType === 'FCL' ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                <div className="overflow-hidden">
                  <InputControl label="컨테이너 비용" name="containerCost" value={formData.containerCost} onChange={handleInputChange} unit="원" icon={<Icons.CurrencyWonIcon />} onKeyDown={handleKeyDown} />
                </div>
              </div>
